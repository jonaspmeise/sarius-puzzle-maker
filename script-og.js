const challenges = [
  ...
  {prompt: (text) => `Password must not be made out of a total number of characters that is a multiple of 3. <i>(Currently: ${text.length} characters)</i>`, check: (text) => text.length % 3 > 0},
  ...
  {prompt: 'Password must not repeat any characters (literally) right after one another.', check: (text) => !/(.)\1/.test(text)},
  ...
  {prompt: 'Babe wake up it\'s ____ Saturday!', check: (text) => convert(/vivy/i).test(text)},  {prompt: (text) => `Password must have a total sum of all digits equal to 5. <i>(Current total sum: ${checksum(text)})</i>`, check: (text) => {
...
function call(e) {
  ...
  if(name !== 'SARIUS' && name !== 'SARIUSCRAST') {
    alert('Bro, you know what your name is, right?');
    document.getElementById("username").value = '';
    return;
  }

  window.location.href = './end.html?finish=' + document.getElementById("inputBox").value;
}
...
function getQueryParams() {
  ...
  const finishWith = document.getElementById("finishWith");
  if(finishWith === null) return;

  finishWith.innerHTML = finish;
}